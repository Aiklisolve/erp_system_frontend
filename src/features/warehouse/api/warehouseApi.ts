...
function mapBackendWarehouse(backendWarehouse: any): Warehouse {
  ...
  return {
    ...
    address: backendWarehouse.address || backendWarehouse.address_line1 || undefined,
...
export async function createWarehouse(
  ...
  if (USE_BACKEND_API) {
    try {
      ...
      // warehouse_code will be auto-generated by backend if not provided
      ...
      // Only include warehouse_code if it's provided (for edit mode)
      if (payload.warehouse_code) {
        backendPayload.warehouse_code = payload.warehouse_code;
      }
...
export async function updateWarehouse(
  ...
  if (USE_BACKEND_API) {
    try {
      ...
      if (changes.name !== undefined) backendPayload.warehouse_name = changes.name;
      if (changes.address !== undefined) backendPayload.address_line1 = changes.address;
